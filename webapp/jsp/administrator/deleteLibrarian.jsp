    ...
    <div class="row" style="height: 97%">
        ...
        <div class="col-xs-12 col-sm-12 col-md-10" style="height: 100%;">
            ...
            <div class="row" style="min-height: 17%; border-radius: 20px; background-color: #424c93">
                ...
                <h1 style="margin-left: 70px;"><fmt:message key="deleteLibrarian" bundle="${booking}"/></h1>
                <div class="col-xs-12 col-sm-12 col-md-12" style="height: 100%">
                    <span style="color: white"><fmt:message key="deleteLibrarianInfo" bundle="${booking}"/></span>
                </div>
            </div>
            <c:if test="${ not empty librarianList}">
                <div class="row" style="min-height: 80%; margin-top: 20px;" >
                    ...
                    <form action="/controller" method="post" id="deleteLibrarianForm">
                        <tr>
                            <td><c:out value="${librarian.name}"/></td>
                            <td><c:out value="${librarian.surname}"/></td>
                            <td><c:out value="${librarian.email}"/></td>
                            <td><c:out value="${librarian.registrationDate}"/></td>
                            <td>
                                <input type="hidden" name="command" value="delete_user">
                                <input type="hidden" name="page" value="deleteLibrarian">
                                <button type="button" class="googleButton" data-toggle="modal" data-target="#${librarian.id}"><fmt:message key="deleteLibrarian" bundle="${booking}"/></button>
                                <div id="${librarian.id}" class="modal fade" role="dialog">
